...
function PANEL:Init()
	...
	self.Label = vgui.Create( "BP_DTree_Node_Button", self )
...
function PANEL:SetExpanded( bExpand, bSurpressAnimation )
	...
	if self.Expander.SetExpanded then self.Expander:SetExpanded( bExpand ) end -- we use checkboxes for function profiling selection, this will error hehe
...
function PANEL:PerformLayout()
	...
	if ( self.m_bHideExpander ) then
		...
		self.Expander:SetSize( 15, 15 )
		self.Expander:SetVisible( self.NeedsLazyLoad || self:HasChildren() || self:GetForceShowExpander() )
...
function PANEL:AddNode( strName, strIcon )
	...
	local pNode = vgui.Create( "BP_DTree_Node", self )
...
function PANEL:Copy()
	...
	local copy = vgui.Create( "BP_DTree_Node", self:GetParent() )
...
derma.DefineControl( "BP_DTree_Node", "Tree Node", PANEL, "DPanel" )